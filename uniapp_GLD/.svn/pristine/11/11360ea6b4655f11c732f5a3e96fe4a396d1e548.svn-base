		...
		<scroll-view :scroll-y="masked" :style="{ height: height }" style="background-color: #f4f7ff;padding:20upx 0;" @scrolltolower="scrolltolower">
			<block v-for="(item, index) in dataList" :key="index">
				<view class="list_box" @click="tzxqy(index)">
					...
					<view class="boxContL">
						<view style="color: #9e9e9e; font-size: 28upx">申请时间: {{ item.Date }}</view>
						<view style="color: #ffaa00;" v-if="tabIndex == 0">审核待处理</view>
						<view style="color: #28c38f;" v-if="tabIndex == 1">审核已同意</view>
					</view>
				</view>
			</block>
...
export default {
	...
	created() {
		...
		that.heightNull = that.getWindowHeight() / 2 - 50 + 'px';
	},
	onShow() {
		this.getList();
	},
		...
		getList(page = 1) {
			...
			api.httpRequestGet(url, data).then(res => {
				if (res.data.error_code == 'Success') {
					...
					if (jkdata.list.length > 0) {
						jkdata.list.forEach(v => {
