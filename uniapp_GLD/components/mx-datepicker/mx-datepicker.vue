<template>
	<view v-show="isShow" class="picker">
		<!-- 日期选择器 -->
		<view v-show="type != 'time'" class="picker-modal">
			...
			<view class="picker-modal-footer">
				<view class="picker-modal-footer-info"></view>
		...
		onConfirm() {
			...
			if (this.isMultiSelect) {
				...
				this.checkeds.forEach((date, index) => {
					...
					values.push(DateTools.format(newDate, this.format ? this.format : defaultFormat[this.isContainTime ? 'datetime' : 'date']));
					dates.push(newDate);
				});
				result.value = values;
				result.date = dates;
