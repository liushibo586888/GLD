	...
	<view class="page">
		...
		<swiper class="swiper" :style="{ height: height }" @change="scollSwiper" :current="current">
			<swiper-item v-for="(item, index) in items" :key="index">
				<scroll-view scroll-y="true" style="height: 100%;box-sizing: border-box;" @scrolltolower="scrolltolower">
					<block v-for="(item, index) in dataList" :key="index">
						<!-- dataList -->
						<view class="HLS_box">
							<view style="background: #FFFFFF;height: 80upx;">
								<view class="HLS_shang">{{ item.ChannelName }} / {{ item.bili == NaN ? 0 : item.bili.toFixed(2) }}%</view>
								<view v-if="Sort == 0">
							...
							<view class="HLS_zhong HLS_xia">
								<view>
									<text style="color: #ffa32f;font-size: 28upx;">{{ item.NewToStore }}</text>
									<text class="fontColor">到店新客</text>
									<text class="xian_h"></text>
								</view>
								<view>
									<text style="color: #8caaff;font-size: 28upx;">{{ item.NewToClinch }}</text>
									<text class="fontColor">新客成交</text>
									<text class="xian_h"></text>
								</view>
								<view>
									<text style="color: #42a5f5;font-size: 28upx;">{{ item.ChannelAmount }}</text>
									<text class="fontColor">总金额</text>
									<text class="xian_h"></text>
								</view>
								<view>
									<text style="color: #4aa8f5;font-size: 28upx;">{{ item.MonomerAmount }}</text>
									<text class="fontColor">客单</text>
								</view>
							</view>
						</view>
					</block>
...
export default {
	...
	methods: {
		...
		getList(page = 1, size = 5) {
			let url = 'StatiStical/ChannelStatistics';
			let data = {
